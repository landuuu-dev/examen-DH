...
function CardCategorias({ categoriaData, onDelete, onEdit }) {
  ...
  // 1. Obtener la lista de imágenes independientemente de cómo venga del backend
  const obtenerListaImagenes = () => {
    // Si la API devuelve un arreglo "imagenes" o "images"
    if (
      Array.isArray(categoriaData.imagenes) &&
      categoriaData.imagenes.length > 0
    ) {
      return categoriaData.imagenes;
    }
    if (
      Array.isArray(categoriaData.images) &&
      categoriaData.images.length > 0
    ) {
      return categoriaData.images;
    }
    // Si viene como propiedades sueltas imagen1, imagen2...
    if (
      categoriaData.imagen1 ||
      categoriaData.imagen2 ||
      categoriaData.imagen3
    ) {
      return [
        categoriaData.imagen1,
        categoriaData.imagen2,
  ...
  // Función para asegurar que la URL sea válida
  const formatUrl = (url) => {
    if (!url) return null;
    if (typeof url !== "string") return null;
    // Si es una ruta relativa del servidor local o render
    if (url.startsWith("/")) {
      ...
      <div>
        ...
        <div className="relative w-full h-48 bg-slate-100 overflow-hidden">
          {imagenPrincipal ? (
            <img
              ...
              onError={(e) => {
                // Previene bucle de error si falla la carga
          ...
          {/* ID Badge en la esquina superior */}
          <span className="absolute top-3 right-3 px-2.5 py-1 bg-slate-900/70 backdrop-blur-xs text-white text-xs font-mono rounded-full shadow-xs">
            #{String(categoriaData.id || "").slice(-4)}
          </span>
        </div>
        ...
        <div className="p-5">
          ...
          <p className="text-slate-600 text-sm line-clamp-3 mb-4 leading-relaxed">
            {categoriaData.descripcion || "Sin descripción disponible."}
          </p>

          {/* Miniaturas de imágenes adicionales si las hay */}
